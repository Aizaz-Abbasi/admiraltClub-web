...
const getPlans = async (_req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ success: false, message: "Could not fetch plans from Stripe." });
  }
};
...
const createCheckoutSession = async (req, res) => {
  try {
    ...
    // Get or create Stripe customer — check membership AND day pass records
    ...
    if (!plan.priceId) {
      return res
        ...
        .json({
          success: false,
          message: `Stripe price not configured for ${type}. Set STRIPE_PRICE_${type} in environment variables.`,
        });
    }
  ...
  try {
    switch (event.type) {
      ...
      case "invoice.paid": {
        ...
        if (!membership) {
          console.error(
            "invoice.paid: no membership for subscription",
            subscriptionId,
          );
          break;
        }
    ...
    // Return 200 so Stripe does not retry — the error is non-recoverable and already logged
...
const verifySession = async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json({ success: true, purchaseType: membershipType, membership });
  } catch (error) {
  ...
  try {
    ...
    if (membership.stripeSubscriptionId) {
      try {
        ...
        // Already cancelled in Stripe — safe to continue with DB update
      }
    }
    ...
    return res
      .status(200)
      .json({ success: true, message: "Membership cancelled." });
  } catch (error) {
...
const createGuestUser = async (req, res) => {
  try {
    ...
    const rawPassword =
      "Day" + Math.random().toString(36).slice(2, 8).toUpperCase();
